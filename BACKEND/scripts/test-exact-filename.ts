/**
 * Teste rápido para verificar nome exato do arquivo
 */
...
async function test() {
  const jsonPath = path.join(__dirname, '../output/scraped/questions/sus-sp-2015-1762048243591.json');
  const jsonContent = await fs.readFile(jsonPath, 'utf-8');
  const questions = JSON.parse(jsonContent);
  
  console.log('🧪 Testando upload com nome exato...\n');
