#!/usr/bin/env ts-node
/**
 * Test script for GLM-4.6 integration
 * 
 * Usage:
 *   ts-node scripts/test-glm.ts
 */

import dotenv from 'dotenv';
import { createGLMClient } from '../src/services/glmClient';

// Load environment variables
dotenv.config();

async function testGLMClient() {
  console.log('🧪 Testing GLM-4.6 Client\n');

  // Check environment variables
  const apiKey = process.env.GLM_API_KEY || process.env.QWEN_API_KEY;
  if (!apiKey) {
    console.error('❌ GLM_API_KEY not found in .env');
    process.exit(1);
  }

  console.log('✅ API Key found');
  console.log(`📝 Model: ${process.env.GLM_MODEL || 'zai-org/GLM-4.6'}`);
  console.log(`📏 Max output tokens: ${process.env.GLM_MAX_CONTEXT || '8192'}\n`);

  // Create client
  const client = createGLMClient(apiKey);
...
Retorne em formato JSON:
{
  ...
  try {
    console.log('⏳ Enviando requisição para GLM-4.6...\n');
...
[
  ...
  try {
    console.log('⏳ Enviando batch para GLM-4.6...\n');
    const startTime = Date.now();
