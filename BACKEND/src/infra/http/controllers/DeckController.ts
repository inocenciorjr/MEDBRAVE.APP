...
import { deckService } from '../../../domain/studyTools/flashcards/services/deckService';

export class DeckController {
  async create(request: Request, response: Response): Promise<Response> {
    const { userId } = request;
    const { name, description, isPublic, tags, coverImageUrl, status } = request.body;
  ...
  async clone(_request: Request, response: Response): Promise<Response> {
    return response.status(501).json({ message: 'Clone de deck não implementado.' });
  }
}
