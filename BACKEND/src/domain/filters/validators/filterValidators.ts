...
export function validateFilterCategory(
  category: any,
...
export function validateCreateFilter(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  if (!name || typeof name !== 'string' || name.trim() === '') {
    res.status(400).json({
      error: 'Nome do filtro é obrigatório e deve ser uma string válida',
    });
    return;
  }
...
export function validateUpdateFilter(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  if (name !== undefined && (typeof name !== 'string' || name.trim() === '')) {
    res
      .status(400)
      .json({ error: 'Nome do filtro deve ser uma string válida' });
    return;
  }
...
export function validateCreateSubFilter(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  if (!name || typeof name !== 'string' || name.trim() === '') {
    res.status(400).json({
      error: 'Nome do subfiltro é obrigatório e deve ser uma string válida',
    });
    return;
  }
...
export function validateUpdateSubFilter(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  if (name !== undefined && (typeof name !== 'string' || name.trim() === '')) {
    res
      .status(400)
      .json({ error: 'Nome do subfiltro deve ser uma string válida' });
    return;
  }
