...
export class PulseAIController {
  private pulseService: PulseAIService;
  ...
  constructor() {
    ...
    this.pulseService = new PulseAIService(this.defaultConfig);
  }
  ...
  analyzeMedicalCase = async (req: Request, res: Response) => {
    try {
      const { medicalCase, userRole = 'student' }: {
      ...
      if (!this.defaultConfig.apiKey) {
        return res.status(500).json({
          success: false,
          error: 'PULSE AI não configurado. Entre em contato com o administrador.',
          code: 'PULSE_NOT_CONFIGURED'
        });
      }

      // Fazer análise
      const result = await this.pulseService.analyzeMedicalCase(
        medicalCase,
        userId,
        userRole
      );
  ...
  educateMedicalTopic = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.pulseService.educateMedicalTopic(
        topic,
        specialty || '',
        complexity,
        userId,
  ...
  quickMedicalQuery = async (req: Request, res: Response) => {
    try {
      ...
      // Consulta rápida
      const result = await this.pulseService.quickMedicalQuery(
        query,
        userId,
        userRole
      );
  ...
  explainQuestionAnswer = async (req: Request, res: Response) => {
    try {
      ...
      // Explicar questão
      const result = await this.pulseService.explainQuestionAnswer(
        question,
        alternatives,
        correctAnswer,
        userAnswer,
    ...
    try {
      ...
      if (req.user?.role !== 'ADMIN') {
        return res.status(403).json({
          success: false,
          error: 'Acesso negado. Apenas administradores podem extrair questões.',
          code: 'ACCESS_DENIED'
        });
      }
      ...
      // Extrair questões
      const result = await this.pulseService.extractQuestionsFromContent(
        content,
        options,
        userId,
        'admin'
  ...
  categorizeQuestions = async (req: Request, res: Response) => {
    try {
      ...
      if (req.user?.role !== 'ADMIN') {
        ...
        return res.status(403).json({
          success: false,
          error: 'Acesso negado. Apenas administradores podem categorizar questões.',
          code: 'ACCESS_DENIED'
        });
      }
      ...
      console.log('  - Primeiros chars da API Key:', this.defaultConfig.apiKey?.substring(0, 10) + '...');
      ...
      if (!this.defaultConfig.apiKey) {
        ...
        return res.status(500).json({
          success: false,
          error: 'PULSE AI não configurado. Configure GOOGLE_AI_API_KEY no arquivo .env',
          code: 'PULSE_NOT_CONFIGURED'
        });
      }
      ...
      const result = await this.pulseService.categorizeQuestions(
        questions,
        availableFilters,
        userId,
        'admin'
  ...
  detectOutdatedQuestions = async (req: Request, res: Response) => {
    try {
      ...
      if (req.user?.role !== 'ADMIN') {
        return res.status(403).json({
          success: false,
          error: 'Acesso negado. Apenas administradores podem detectar questões desatualizadas.',
          code: 'ACCESS_DENIED'
        });
      }
      ...
      const { questionData, userRole = 'admin', options = {} } = req.body;
      ...
      let parsedQuestions;
      try {
        parsedQuestions = typeof questionData === 'string' ? JSON.parse(questionData) : questionData;
      } catch (e) {
        return res.status(400).json({
          success: false,
      ...
      console.log(`📊 Analisando ${questionsArray.length} questões para protocolos desatualizados`);
      ...
      if (validQuestions.length === 0) {
        return res.status(400).json({
          success: false,
          error: 'Nenhuma questão válida encontrada (precisam ter enunciado, alternativas e resposta correta)',
          code: 'NO_VALID_QUESTIONS'
        });
      }

      console.log(`✅ ${validQuestions.length} questões válidas de ${questionsArray.length} totais`);
      ...
      for (const [index, question] of validQuestions.entries()) {
        try {
          console.log(`🔍 Analisando questão ${index + 1}/${validQuestions.length}: ${question.statement?.substring(0, 100)}...`);
...
Seja conservador - apenas marque como desatualizada se tiver certeza de mudanças significativas.`;
          ...
          try {
            ...
            if (jsonMatch) {
              ...
              const isOutdated = result.content.toLowerCase().includes('desatualizada') || 
                               result.content.toLowerCase().includes('mudou') ||
                               result.content.toLowerCase().includes('atualizado');
            ...
            console.warn(`⚠️ Erro ao parsear JSON para questão ${index + 1}, usando fallback`);
            analysisResult = {
              isOutdated: false,
              confidence: 0.5,
          ...
          if (analysisResult.isOutdated && analysisResult.confidence > 0.6) {
            ...
            console.log(`📅 Questão ${index + 1} marcada como DESATUALIZADA (${Math.round(analysisResult.confidence * 100)}% confiança)`);
          } else {
            console.log(`✅ Questão ${index + 1} parece ATUALIZADA (${Math.round(analysisResult.confidence * 100)}% confiança)`);
          }
      ...
      console.log(`✅ Detecção concluída: ${outdatedQuestions.length} questões desatualizadas de ${validQuestions.length} analisadas`);
  ...
  moderateMedicalContent = async (req: Request, res: Response) => {
    try {
      ...
      // Moderação
      const result = await this.pulseService.moderateMedicalContent(
        content,
        userId,
        context
      );
  ...
  getStatus = async (req: Request, res: Response) => {
    try {
      const status = this.pulseService.getStatus();
      
      // Verificar saúde do sistema
      const health = {
        status: status.ready ? 'healthy' : 'unhealthy' as 'healthy' | 'unhealthy',
  ...
  updateConfiguration = async (req: Request, res: Response) => {
    try {
      ...
      if (newConfig.temperature && (newConfig.temperature < 0 || newConfig.temperature > 2)) {
        return res.status(400).json({
          success: false,
      ...
      if (newConfig.maxTokens && (newConfig.maxTokens < 100 || newConfig.maxTokens > 8192)) {
        return res.status(400).json({
          success: false,
      ...
      this.pulseService.updateConfiguration(newConfig);
  ...
  getAnalytics = async (req: Request, res: Response) => {
    try {
      ...
      const logger = (this.pulseService as any).logger;
   ...
   */
  testSystem = async (req: Request, res: Response) => {
    try {
      // Verificar ambiente
  ...
  generateQuestionExplanation = async (req: Request, res: Response) => {
    try {
      ...
      if (questionData.correctAnswerIndex === undefined || questionData.correctAnswerIndex < 0) {
        return res.status(400).json({
          success: false,
      ...
      if (!forceRegenerate) {
        ...
        console.log('💾 Cache não implementado ainda, gerando nova explicação...');
      }
...
ANÁLISE INDEPENDENTE DE QUESTÃO MÉDICA:
...
ALTERNATIVAS:
${questionData.alternatives.map((alt: string, i: number) => 
...
IMPORTANTE: Base sua análise apenas em evidências médicas científicas atuais. Ignore qualquer indicação prévia de qual seria a "resposta correta".`;
      ...
      const validationResult = await this.pulseService.quickMedicalQuery(
        validationPrompt,
        userId,
        userRole
      );
      ...
      if (validationResult.success) {
        try {
          ...
          const aiAnswerMatch = validationResult.content.match(/ALTERNATIVA_CORRETA:\s*([A-Z])/i);
          const justificationMatch = validationResult.content.match(/JUSTIFICATIVA:\s*(.+)/i);
          
          if (aiAnswerMatch) {
            aiSuggestedAnswer = aiAnswerMatch[1].toUpperCase();
            const expectedAnswer = String.fromCharCode(65 + questionData.correctAnswerIndex);
            ...
            if (!aiAnalysisMatch) {
              ...
              console.log(`✅ Validação OK: IA concorda com alternativa ${expectedAnswer}`);
            }
          }
        } catch (e) {
        ...
        console.warn('⚠️ Falha na validação prévia, continuando com resposta marcada...');
      }
...
`;
      ...
      const explanationMetadata = {
        ...
        validation: {
          performed: true,
          aiAnalysisMatch,
          markedAnswer: String.fromCharCode(65 + questionData.correctAnswerIndex),
      ...
      console.log(`✅ Explicação gerada com sucesso (${generatedExplanation.length} caracteres)`);
  ...
  convertDocumentToMarkdown = async (req: Request, res: Response) => {
    try {
      ...
      if (req.user?.role !== 'ADMIN') {
        return res.status(403).json({
          success: false,
          error: 'Acesso negado. Apenas administradores podem converter documentos.',
          code: 'ACCESS_DENIED'
        });
      }
      ...
      const result = await this.pulseService.convertDocumentToMarkdown(
        content,
        documentType,
        options,
        userId,
