...
class Config:
    ...
    FILTERS_JSON_PATH: str = os.environ.get("FILTERS_JSON_PATH", "/home/ubuntu/estrategia_filters_extracted.json")
    ...
    def filters_endpoint(self) -> str:
        return f"{self.MEDPULSE_API_BASE_URL}/filters"
    
    @property
    def subfilters_endpoint_template(self) -> str:
        return f"{self.MEDPULSE_API_BASE_URL}/filters/{{filter_id}}/subfilters"
    
    @property
    def auth_endpoint(self) -> str:
        return f"{self.MEDPULSE_API_BASE_URL}/auth/login"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('filter_import.log'),
...
# --- Data Validation ---
def validate_json_structure(data: List[Dict]) -> bool:
    """Validates the JSON structure before processing."""
    if not isinstance(data, list):
        logger.error("Root data must be a list")
        return False
    ...
    for i, item in enumerate(data):
        ...
        if "especialidade" not in item:
            logger.error(f"Item {i} missing 'especialidade' field")
            return False
        
        subespecialidades = item.get("subespecialidades", [])
        if not isinstance(subespecialidades, list):
            logger.error(f"Item {i} 'subespecialidades' must be a list")
            return False
...
def import_subfilters_recursive(
    ...
    for subfilter_item in subfilter_list:
        ...
        subfilter_name = (
            subfilter_item.get("nome") or 
            subfilter_item.get("assunto") or
            (subfilter_item if isinstance(subfilter_item, str) else None)
        )
        
        children = subfilter_item.get("assuntos", []) if isinstance(subfilter_item, dict) else []
        
        # Convert string arrays to dict format
...
# --- Main Execution ---
def main():
    logger.info("=== Starting MedPulse Hierarchical Filter Import ===")
    ...
    try:
        ...
        logger.info(f"Loaded {len(data_to_import)} especialidades from JSON file")
    except Exception as e:
        logger.error(f"Error reading JSON file: {e}")
        return False

    if not validate_json_structure(data_to_import):
    ...
    logger.info(f"Especialidades Skipped (existing): {especialidades_skipped}")
    logger.info(f"Subfiltros Created: {total_stats.created}")
    logger.info(f"Subfiltros Skipped (existing): {total_stats.duplicate}")
    logger.info(f"Subfiltros Skipped (invalid): {total_stats.skipped}")
