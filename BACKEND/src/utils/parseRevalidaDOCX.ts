...
export async function extractQuestionsFromDocx(buffer: Buffer): Promise<ExtractedDocxResult> {
  ...
  const mammothOptions = {
    convertImage: (mammoth.images as any).inline(async (element: any) => {
      ...
      return { src: `data:${element.contentType};base64,${(await imageBuffer).toString('base64')}` };
    })
  };

  const { value: html } = await mammoth.convertToHtml({ buffer }, mammothOptions);
  console.log('🔍 HTML gerado pelo Mammoth (primeiros 5000):', html.slice(0, 5000));
  ...
  if (images.length > 0) {
    const sortedImages = images.slice().sort((a, b) => b.buffer.length - a.buffer.length);
    sortedImages.slice(0, 5).forEach((img, idx) => {
      console.log(`🖼️ TOP${idx + 1} Imagem: ${img.filename}, tipo: ${img.type}, tamanho: ${img.buffer.length} bytes`);
    });
  }
  ...
  for (let i = 0; i < questionTitles.length; i++) {
    ...
    const next = i < questionTitles.length - 1 ? questionTitles[i + 1] : {index: allParagraphs.length};
    
    // Define limites da questão atual
    const startIdx = current.index;
    const endIdx = next.index;
    
    console.log(`⚙️ Processando questão ${i+1}: "${current.title}" (índices ${startIdx}-${endIdx})`);
    ...
    for (let j = 0; j < paragraphTexts.length; j++) {
      ...
      if (altPattern1.test(text) || altPattern2.test(text)) {
        ...
        if (letter && content) {
          console.log(`📌 Alternativa detectada: ${letter} - "${content.substring(0, 30)}..."`);
    ...
    const firstAltIdx = alternativeIndices.length > 0 ? alternativeIndices[0] : paragraphTexts.length;
    ...
    for (let j = 0; j < statementParagraphs.length; j++) {
      ...
      if (/^(Exame|Parâmetro|Frequência|Pressão|Temperatura|Saturação|Hemoglobina|Hematócrito|Leucócitos|Volume|Plaquetas|Teste|Ferritina|Batimentos|Altura|Capacidade|Relação|VEF1|CVF)\b/.test(text) ||
          /^(Valor|Resultado|Referência)/.test(text) || 
          /^[0-9,.]+\s*(mg\/dL|g\/dL|%|bpm|mmHg|fL|pg|\/mm3?|irpm|°C|cm)$/.test(text)) {
        
        examData.push(text);
        console.log(`📊 Dado de exame detectado: "${text.substring(0, 30)}..."`);
      }
      // Se for título da questão ou texto que não é dado de exame
      else if (/QUEST(Ã|A)O\s+\d+/i.test(text) || !text.match(/^(Exame|Valor|Resultado|Referência|Parâmetro)/)) {
    ...
    for (const table of foundTables) {
      ...
      for (const examLine of examData) {
        if (tableText.includes(examLine)) {
          ...
          console.log(`📋 Tabela associada à questão ${i+1} por dados de exame`);
          tableMatched = true;
          break;
        }
      }
    ...
    for (let imgIdx = 0; imgIdx < images.length; imgIdx++) {
      ...
      const imgRegex = new RegExp(`<img[^>]*src=["'][^"']*${img.id}[^"']*["'][^>]*>`, 'i');
      ...
      if (imgHtmlMatch) {
        ...
        if (Math.abs(imgPosition - questionPosition) < proximityThreshold) {
          ...
          console.log(`🖼️ Imagem ${img.id} associada à questão ${i+1} por proximidade`);
        }
      }
    }
    ...
    if (statement || alternatives.length > 0) {
      ...
      const expectedAlternatives = ['A', 'B', 'C', 'D', 'E'].slice(0, alternatives.length);
      let alternativesOk = true;
      
      // Se tivermos pelo menos duas alternativas, verificamos a sequência
      if (alternatives.length >= 2) {
        console.log(`   Verificando sequência de alternativas: ${alternatives.length} alternativas`);
        ...
        console.log(`   Letras das alternativas detectadas: ${alternativeLetters.join(', ')}`);
        
        // Verifica se as alternativas estão em ordem alfabética
        for (let k = 0; k < alternativeLetters.length - 1; k++) {
          if (alternativeLetters[k].charCodeAt(0) + 1 !== alternativeLetters[k + 1].charCodeAt(0)) {
            alternativesOk = false;
            console.log(`⚠️ Sequência de alternativas incorreta: ${alternativeLetters.join(', ')}`);
            break;
          }
        }
        ...
        if (!alternativesOk) {
          ...
          console.log(`✅ Alternativas reorganizadas: ${alternatives.length} alternativas`);
        }
      }
      
      questionBlocks.push(question);
      console.log(`✅ Questão ${i+1} extraída com ${alternatives.length} alternativas`);
      ...
      alternatives.forEach((alt, idx) => {
        console.log(`   - Alt ${String.fromCharCode(65 + idx)}: "${alt.substring(0, 30)}..."`);
      });
    } else {
      console.log(`⚠️ Questão ${i+1} ignorada - enunciado vazio e sem alternativas`);
    }
  }
