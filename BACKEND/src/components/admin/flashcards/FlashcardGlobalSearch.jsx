...
const FlashcardGlobalSearch = ({ onResultsChange, onLoadingChange }) => {
  ...
  const loadFSRSStatus = async () => {
    try {
      ...
      const response = await fetchWithAuth('/api/admin/flashcards/fsrs-status');
  ...
  const performSearch = async (query, filters) => {
    try {
      ...
      const response = await fetchWithAuth(`/api/admin/flashcards/search?${params}`);
      
      if (response.success) {
        setSearchResults(response.data.results);
        onResultsChange && onResultsChange(response.data.results);
      } else {
        setSearchResults([]);
        onResultsChange && onResultsChange([]);
      }
