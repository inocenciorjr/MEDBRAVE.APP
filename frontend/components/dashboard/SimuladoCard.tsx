...
export default function SimuladoCard({ simulado }: SimuladoCardProps) {
  ...
  return (
    <div
      className={`${colors.bg} p-3 sm:p-4 rounded-lg shadow-xl dark:shadow-dark-xl hover:shadow-2xl dark:hover:shadow-dark-2xl hover:scale-[1.02] transition-all duration-300 cursor-pointer`}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-2">
        <div>
          <h3 className="font-bold text-sm sm:text-base text-text-light-primary dark:text-text-dark-primary">
            {simulado.title}
          </h3>
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
        {simulado.tags.map((tag) => (
          <span
            key={tag.id}
            className={`text-[10px] sm:text-xs font-medium ${tag.color} px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full`}
          >
            {tag.label}
          </span>
        ))}
      ...
      <div className={`w-full ${colors.progressBg} rounded-full h-1 sm:h-1.5 mb-3 sm:mb-4`}>
        <div
      ...
      {/* Footer */}
      <div className="flex flex-col xs:flex-row justify-between gap-1 xs:gap-0 text-[10px] sm:text-sm">
        <div className="flex items-center gap-1 sm:gap-2 text-text-light-secondary dark:text-text-dark-secondary">
          <span className={`material-symbols-outlined text-sm sm:text-base ${iconColor}`}>
            folder
          </span>
