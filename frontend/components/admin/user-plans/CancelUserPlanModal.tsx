...
export function CancelUserPlanModal({
  userPlan,
  isOpen,
  onClose,
  onConfirm,
}: CancelUserPlanModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao cancelar plano');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminModal
      isOpen={isOpen}
      onClose={onClose}
      title="Cancelar Plano de Usuário"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-text-light-secondary dark:text-text-dark-secondary space-y-1">
            ...
            <p>Status: <strong>{userPlan.status}</strong></p>
          </div>
        </div>
        ...
        <p className="text-sm text-text-light-tertiary dark:text-text-dark-tertiary">
          O usuário perderá acesso aos recursos do plano imediatamente após o cancelamento.
        </p>
