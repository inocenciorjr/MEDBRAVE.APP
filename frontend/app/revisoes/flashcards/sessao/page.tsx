...
export default function FlashcardsRevisaoPage() {
  ...
  const initSession = async () => {
    try {
      ...
      // Buscar revisões pendentes de flashcards de hoje
      ...
      const reviewIds = flashcardReviews.map((r: any) => r.id);
      const token = session.access_token;

      // Verificar se já existe uma sessão ativa de hoje
      const activeSessionResponse = await fetch(`/api/review-sessions/active?contentType=FLASHCARD`, {
        headers: { 
      ...
      router.push(`/revisoes/flashcards/sessao/${sessionId}`);
    } catch (error) {
