...
async function handleRequest(
  ...
  try {
    ...
    console.log(`[Proxy /api/unified-reviews] Fetching from backend: ${url}`);
    const response = await fetch(url, options);
    console.log(`[Proxy /api/unified-reviews] Backend response status:`, response.status);

    // Tentar parsear como JSON
    const contentTypeResponse = response.headers.get('content-type');
    if (contentTypeResponse?.includes('application/json')) {
      const data = await response.json();
      console.log(`[Proxy /api/unified-reviews] Backend response data:`, data);
