...
async function handleRequest(
  ...
  try {
    ...
    console.log(`[Proxy /api/auth] Fetching from backend: ${url}`);
    const response = await fetch(url, options);
    console.log(`[Proxy /api/auth] Backend response status:`, response.status);

    // Tentar parsear como JSON
    const contentTypeResponse = response.headers.get('content-type');
    if (contentTypeResponse?.includes('application/json')) {
      const data = await response.json();
      console.log(`[Proxy /api/auth] Backend response data:`, data);
    ...
    console.error('[Proxy /api/auth] Error:', error);
