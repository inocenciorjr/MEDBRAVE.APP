...
export default function ReviewErrorNotebookPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const [studiedEntries, setStudiedEntries] = useState<Map<string, 'again' | 'hard' | 'good' | 'easy'>>(new Map());
  ...
  // Carregar sessão de estudo do sessionStorage
  useEffect(() => {
    const loadSession = () => {
      try {
        // Buscar IDs do sessionStorage
        const storedIds = sessionStorage.getItem(`study-session-${sessionId}`);
        ...
        if (ids.length === 0) {
          alert('Não há itens para estudar!');
          router.push('/caderno-erros');
          return;
        }
  ...
  const { data: entryData } = useCadernoErrosEntry(entryIds[currentIndex] || '');
    ...
    if (entryData) {
      ...
      if (entryData.question_id) {
        fetchWithAuth(`/api/questions/${entryData.question_id}`)
          ...
          .catch(err => console.error('Erro ao carregar questão:', err));
      }
    }
  }, [entryData]);

  // Carregar entry atual (fallback)
  useEffect(() => {
    const loadEntry = async () => {
      ...
      try {
        ...
        if (foundEntry) {
          ...
          // Buscar dados completos da questão
          try {
            const response = await fetchWithAuth(`/api/questions/${foundEntry.question_id}`);
            if (response.ok) {
              const questionData = await response.json();
              setFullQuestion(questionData.data);
            }
  ...
  const handleStudy = async (difficulty: 'again' | 'hard' | 'good' | 'easy') => {
    try {
      const entryId = entryIds[currentIndex];
      
      // Marcar como estudado
      setStudiedEntries(prev => new Map(prev).set(entryId, difficulty));
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 overflow-auto -m-4 md:-m-8 min-h-screen bg-gradient-to-br from-gray-100 via-gray-50 to-gray-100 dark:from-black dark:via-background-dark dark:to-black">
        <div className="flex gap-6 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark p-6 shadow-lg hover:shadow-xl dark:shadow-dark-lg dark:hover:shadow-dark-xl transition-shadow duration-300">
            ...
            <DifficultyButtons
              ...
              selectedDifficulty={studiedEntries.get(entryIds[currentIndex])}
              disabled={studiedEntries.has(entryIds[currentIndex])}
              onCardDeleted={() => {
