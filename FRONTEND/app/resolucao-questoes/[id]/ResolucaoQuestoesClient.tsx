...
import { ResolucaoQuestoesPageSkeleton } from '@/components/skeletons/ResolucaoQuestoesPageSkeleton';
...
function ResolucaoQuestoesContent({ id }: ResolucaoQuestoesClientProps) {
  ...
  const { startSession, endSession, incrementItems } = useStudySession('questions');
  ...
  const loadBatch = useCallback(async (extractedListId: string, batchIndex: number) => {
    ...
    try {
      ...
      // Se é modo revisão, carregar pelos IDs
      if (isReviewMode) {
        const batchIds = questionIds.slice(offset, offset + QUESTIONS_PER_BATCH);
        const { getQuestionsByIds } = await import('@/lib/api/questions');
        batchQuestions = await getQuestionsByIds(batchIds);
      } else {
        // Modo normal: carregar da lista
        ...
        if (response.total && response.total !== totalQuestions) {
  ...
  useEffect(() => {
    async function init() {
      // Verificar se é uma revisão (tem parâmetro ids na URL)
      const urlParams = new URLSearchParams(window.location.search);
      const idsParam = urlParams.get('ids');
      const isReview = !!idsParam;
      ...
      try {
        // Se é revisão, carregar questões pelos IDs
        if (isReview && idsParam) {
          setIsReviewMode(true);
          const ids = idsParam.split(',');
          setQuestionIds(ids);
          setTotalQuestions(ids.length);
          setListName('Revisão de Questões');

          // Carregar primeiro batch de questões
          const batchIndex = Math.floor(initialIndex / QUESTIONS_PER_BATCH);
          const offset = batchIndex * QUESTIONS_PER_BATCH;
          const batchIds = ids.slice(offset, offset + QUESTIONS_PER_BATCH);

          const { getQuestionsByIds } = await import('@/lib/api/questions');
          const batchQuestions = await getQuestionsByIds(batchIds);

          // Pré-carregar stats do batch
          await preloadStats(batchIds);

          // Inicializar array com tamanho correto
          const emptyArray: (Question | null)[] = Array.from({ length: ids.length }, () => null);
        ...
        // Fluxo normal: carregar de uma lista
        const batchIndex = Math.floor(initialIndex / QUESTIONS_PER_BATCH);
        const offset = batchIndex * QUESTIONS_PER_BATCH;
  ...
  useEffect(() => {
    // Em modo revisão, precisa ter questionIds carregados
    if (isReviewMode && questionIds.length === 0) return;
    // Em modo normal, precisa ter listId
    if (!isReviewMode && !listId) return;
  ...
  }, [currentIndex, listId, totalQuestions, loadedBatches, loadBatch, isReviewMode, questionIds]);

  // Esperar tanto as questões quanto os stats carregarem
  const isLoadingStats = loadingStats.size > 0;

  if (loading || isLoadingStats) {
    return <ResolucaoQuestoesPageSkeleton />;
  }
