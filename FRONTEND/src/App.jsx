...
import './App.css';
import { Sun, Moon, MoreHorizontal, RotateCcw, Calendar, Play, Pause, Video, BookOpen, X, Filter, BarChart3, FileText, Gem, User as UserIcon } from 'lucide-react';
import { 
  CustomDashboardIcon, 
  CustomRevisionIcon, 
  CustomFlashcardIcon, 
...
// 🚀 LAZY LOADING: Componentes de página carregados dinamicamente
const QuestoesPageNew = lazy(() => import('./pages/dashboard/QuestoesPage'));
const QuestoesResolverPage = lazy(() => import('./pages/dashboard/QuestoesResolverPage'));
const QuestionListsPage = lazy(() => import('./pages/dashboard/QuestionListsPage'));
const FlashcardsPage = lazy(() => import('./pages/dashboard/flashcards/FlashcardsPage'));
const RevisoesPage = lazy(() => import('./pages/revisoes/RevisoesPage'));

const ReviewSessionPage = lazy(() => import('./components/review/ReviewSessionPage'));
const ReviewSessionConfigPage = lazy(() => import('./pages/review/ReviewSessionConfigPage'));
const RetentionDashboard = lazy(() => import('./pages/dashboard/RetentionDashboard'));
const PlannerDashboard = lazy(() => import('./components/planner/PlannerDashboard'));

// Componentes do Dashboard
import TodayReviewsCard from './components/dashboard/TodayReviewsCard';
// const PlannerCard = lazy(() => import('./components/planner/PlannerCard')); // Removido - só usado na página específica

// 🚀 LAZY LOADING: Páginas adicionais do projeto (removidas as que não existem)
...
const PageLoadingFallback = () => (
  <div className="flex items-center justify-center h-96">
    <div className="text-center">
      ...
      <p className="text-sm" style={{color: 'var(--text-secondary)'}}>Carregando página...</p>
    </div>
  </div>
);
...
const DashboardApp = () => {
  ...
  return (
      ...
      <div className="absolute left-8 top-32 flex flex-col gap-4 z-10">
        {menuItems.map((item) => (
          <MenuButton
            ...
            onClick={async () => {
              // Só navega se for um menu diferente do atual
              if (currentPage !== item.id) {
                console.log(`🚀 [App] Disparando evento selectivePageRefresh para: ${item.id}`);
                // Disparar evento para atualizar apenas a página de destino
                window.dispatchEvent(new CustomEvent('selectivePageRefresh', {
                  detail: { targetPage: item.id }
                }));
              ...
              // Se clicar no mesmo menu, não faz nada (não navega/recarrega)
            }}
          />
        ))}
      </div>
      ...
      <div className={`pl-28 pr-4 py-6 flex-1 flex flex-col ${currentPage === 'questoes' ? 'overflow-hidden' : 'overflow-auto'}`} style={{width: '100%', boxSizing: 'border-box'}}>
        ...
        {/* 🚀 LAZY LOADING: Páginas com Suspense */}
        {/* Planner Page */}
        {currentPage === 'planner' && (
          <Suspense fallback={<PageLoadingFallback />}>
            <PlannerDashboard userId={user?.uid} />
          </Suspense>
        )}
        ...
        {currentPage === 'questoes-resolver' && (
          <Suspense fallback={<PageLoadingFallback />}>
            <QuestoesResolverPage />
          </Suspense>
        )}
        ...
        {/* Review Session Config Page */}
        {currentPage === 'review-session/config' && (
          <Suspense fallback={<PageLoadingFallback />}>
            <ReviewSessionConfigPage />
          </Suspense>
        )}
        ...
        {!['dashboard', 'questoes', 'listas-questoes', 'questoes-resolver', 'flashcards', 'revisoes', 'review-session', 'review-session/config', 'retencao', 'planner', 'simulados', 'estatisticas', 'conquistas', 'perfil', 'configuracoes'].includes(currentPage) && (
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
