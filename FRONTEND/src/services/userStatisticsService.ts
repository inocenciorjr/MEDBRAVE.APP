...
class UserStatisticsService {
  ...
  async getUserStatistics(userId: string, options?: {
    ...
    try {
      console.log(`📊 [Frontend Service] Fetching main statistics for user ${userId} with options:`, options);
      ...
      const url = `${this.baseApiUrl}/user/${userId}/statistics?${queryParams.toString()}`;
   ...
   */
  async getSpecialtyPerformance(userId: string): Promise<SpecialtyPerformanceData> {
    try {
      console.log(`🏥 [Frontend Service] Fetching specialty performance for user ${userId}`);
      const url = `${this.baseApiUrl}/dashboard/specialty-performance/${userId}`;
      const response = await fetchWithAuth(url);
   ...
   */
  async getPerformanceHistory(userId: string, days: number = 30): Promise<PerformanceHistoryData[]> {
    try {
      console.log(`📈 [Frontend Service] Fetching performance history for user ${userId} (last ${days} days)`);
      const url = `${this.baseApiUrl}/dashboard/performance-history/${userId}?days=${days}`;
      const response = await fetchWithAuth(url);
   ...
   */
  async getStudyTimeData(userId: string): Promise<StudyTimeData> {
    try {
      console.log(`⏱️ [Frontend Service] Fetching study time data for user ${userId}`);
      const url = `${this.baseApiUrl}/dashboard/study-time/${userId}`;
      const response = await fetchWithAuth(url);
   ...
   */
  async triggerMetricsRecalculation(userId: string): Promise<UserStatistics> {
    try {
      console.log(`🔄 [Frontend Service] Triggering metrics recalculation for user ${userId}`);
      // Use the main statistics endpoint with the forceRecalculate flag
      const recalculatedStats = await this.getUserStatistics(userId, { forceRecalculate: true });
  ...
  async exportUserData(userId: string, format: 'json' | 'csv' = 'json'): Promise<string> {
    try {
      console.log(`📤 [Frontend Service] Requesting user data export for user ${userId} in ${format} format`);
      // Assuming the backend provides an export endpoint
      const url = `${this.baseApiUrl}/users/${userId}/statistics/export?format=${format}`;
      const response = await fetchWithAuth(url);
