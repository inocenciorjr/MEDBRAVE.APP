...
async function processBulkPDF(pdfFile: File): Promise<BulkQuestionResponse> {
  ...
  const response = await fetchWithAuth(`${API_BASE_URL}/api/questions/bulk-from-pdf-optimized`, {
    method: 'POST',
    body: formData,
    credentials: 'include',
  });
...
async function processRevalidaHTML(htmlFile: File): Promise<BulkQuestionResponse> {
  ...
  const response = await fetchWithAuth(`${API_BASE_URL}/api/questions/bulk-from-html-revalida-test`, {
    method: 'POST',
    body: formData,
    credentials: 'include',
  });
