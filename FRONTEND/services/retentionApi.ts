...
export class RetentionApiService {
  ...
  static async getQuestionRetentionHistory(question_id: string) {
    const response = await fetchWithAuth(`${API_BASE}/retention/question/${question_id}/history`);
    if (!response.ok) {
  ...
  static async addQuestionsToFSRS(
    ...
    const response = await fetchWithAuth(`${API_BASE}/retention/fsrs/add`, {
      ...
      body: JSON.stringify({ question_ids, strategy }),
    });
    if (!response.ok) {
